...
function StatInput({ label, value, onChange }: { label: string; value: string; onChange: (v: string) => void }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.45rem 0', borderBottom: '1px solid #1e1e1e' }}>
...
export default function EntryPage() {
  ...
  const [selfStats, setSelfStats] = useState(emptyStats())
  const [peerGrades, setPeerGrades] = useState<Grades>(emptyGrades())
  const [peerStats, setPeerStats] = useState(emptyStats())
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
          ...
          <div className="section-panel">
            <h2 style={{ color: '#e0e0e0', fontWeight: '700', fontSize: '0.875rem', letterSpacing: '0.08em', margin: '0 0 1rem 0', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <span style={{ width: '8px', height: '8px', borderRadius: '50%', background: '#6b0000', display: 'inline-block' }} />
              MY STATS — {playerName.split(' ')[0].toUpperCase()}
            </h2>
            {statRows.map(([k, label]) => <StatInput key={k} label={label} value={selfStats[k]} onChange={v => setSelfStat(k, v)} />)}
            <div style={{ marginTop: '1.25rem', paddingTop: '1.25rem', borderTop: '1px solid #222' }}>
              <p style={{ color: '#6b0000', fontSize: '0.7rem', letterSpacing: '0.12em', margin: '0 0 0.875rem 0', fontWeight: '700' }}>MY SELF-EVALUATION</p>
          ...
          <div className="section-panel">
            ...
            <div style={{ marginBottom: '1rem' }}>
              <label style={{ color: '#555', fontSize: '0.7rem', letterSpacing: '0.1em', display: 'block', marginBottom: '0.4rem' }}>EVALUATING PLAYER</label>
            ...
            {peerName ? (
              <>
                {/* Peer stats */}
                <p style={{ color: '#3b82f6', fontSize: '0.7rem', letterSpacing: '0.1em', margin: '0 0 0.5rem 0', fontWeight: '700' }}>
                  STATS FOR {peerName.split(' ')[0].toUpperCase()}
                </p>
                {statRows.map(([k, label]) => <StatInput key={k} label={label} value={peerStats[k]} onChange={v => setPeerStat(k, v)} />)}

                {/* Peer grades */}
                <div style={{ marginTop: '1.25rem', paddingTop: '1.25rem', borderTop: '1px solid #222' }}>
                  <p style={{ color: '#3b82f6', fontSize: '0.7rem', letterSpacing: '0.12em', margin: '0 0 0.875rem 0', fontWeight: '700' }}>
                    GRADES FOR {peerName.split(' ')[0].toUpperCase()}
                  </p>
            ...
            {/* Coach section */}
            {role === 'coach' && (
              <div style={{ marginTop: '1.5rem', paddingTop: '1.25rem', borderTop: '1px solid #222' }}>
                <p style={{ color: '#800000', fontSize: '0.7rem', letterSpacing: '0.12em', margin: '0 0 0.875rem 0', fontWeight: '700' }}>⚑ COACH GRADES — {playerName.split(' ')[0].toUpperCase()}</p>
        ...
        <div className="section-panel" style={{ marginBottom: '1rem' }}>
          ...
          <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Film notes, timestamps, observations..."
            rows={3} style={{ width: '100%', background: '#222', border: '1px solid #333', borderRadius: '0.5rem', color: '#e0e0e0', padding: '0.625rem 0.875rem', fontSize: '0.875rem', resize: 'vertical', outline: 'none', boxSizing: 'border-box' }} />
        </div>
