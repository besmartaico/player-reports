...
export default function RosterPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/players')
      ...
      .catch(e => { setError('Failed to load players: ' + e.message); setLoading(false) })
  }, [router])
      ...
      <div style={{ maxWidth: '720px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
          ...
          <button
            ...
            style={{ background: 'transparent', border: '1px solid #3a3a3a', borderRadius: '0.5rem', color: '#9e9e9e', padding: '0.4rem 0.875rem', fontSize: '0.8rem', cursor: 'pointer', letterSpacing: '0.05em' }}
          >LOGOUT</button>
        </div>

        {/* Filters */}
        <div style={{ background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: '0.75rem', padding: '1rem 1.25rem', marginBottom: '1.25rem' }}>
          <label style={{ color: '#616161', fontSize: '0.7rem', letterSpacing: '0.12em', display: 'block', marginBottom: '0.625rem' }}>FILTER BY POTENTIAL TEAM PRIMARY</label>
        ...
        {loading && (
          <div style={{ textAlign: 'center', padding: '3rem', color: '#616161' }}>
            <div style={{ fontSize: '0.85rem', letterSpacing: '0.1em' }}>▋ LOADING ROSTER...</div>
          </div>
        )}

        {error && (
          <div style={{ background: '#2d0000', border: '1px solid #800000', borderRadius: '0.5rem', padding: '0.875rem 1rem', marginBottom: '1rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
          {filtered.map(player => (
            <div
              key={player.name}
              onClick={() => router.push('/entry/' + encodeURIComponent(player.name))}
              className="player-card"
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.875rem' }}>
                <div style={{
                  width: '2.25rem', height: '2.25rem', borderRadius: '50%',
                  background: teamColor(player.potentialTeam),
                ...
                <div>
                  ...
                  <p style={{ color: '#616161', fontSize: '0.75rem', margin: 0, letterSpacing: '0.04em' }}>{player.potentialTeam}</p>
                </div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <span style={{ color: '#616161', fontSize: '0.8rem' }}>
                  {role === 'coach' ? 'GRADE' : 'ENTER DATA'}
                </span>
